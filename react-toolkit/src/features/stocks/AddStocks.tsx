...
import { CREATE_REQUEST_FOR_ADDSTOCKFORM_ASYNC, IAddStockApp, SET_DATA } from "./slice/newRequestsSlice";
import { IAddStocksForm } from "./types";
import ListStocks from "./ListStocks";

const schema = z.object({
  name: z.string().min(3, { message: "Name must be atleast 3 characters" }).max(50, { message: "This name cannot exceed more than 50 characters" }),
  symbol: z
    .string()
...
const AddStocks: React.FC = () => {
  ...
  const currentRequest: IAddStockApp = useAppSelector(state => state.allRequest.stock.currentRequest);
  ...
  const onSubmit = (data: FieldValues) => {
    ...
    if (payloadData) {
      ...
      dispatch(CREATE_REQUEST_FOR_ADDSTOCKFORM_ASYNC(payloadData));
      reset();
    }
  };
      ...
      <div>
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="mb-3">
            ...
            <input id="name" type="text" {...register("name")} className="form-control" />
          ...
          <div className="mb-3">
            ...
            <input id="symbol" type="text" {...register("symbol")} className="form-control" />
            {errors.symbol && <p className="text-danger">{errors.symbol.message}</p>}
          </div>
          <div className="mb-3">
            <label htmlFor="description" className="for-label">
              Description :
            </label>
            <input id="description" type="text" {...register("description")} className="form-control" />
          ...
          <button
            type="submit"
            disabled={(errors.name || errors.symbol || errors.description || errors.country) && !isValid}
